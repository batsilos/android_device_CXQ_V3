...
/*---------------------------------------------------------------------------*/
#if defined(ANDROID)
#define  MND_ERR(...)  __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, __VA_ARGS__)
#define  MND_MSG(...)  __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, __VA_ARGS__)

//#define  MND_ERR(fmt, arg ...)  __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "%s", fmt)
//#define  MND_MSG(fmt, arg ...)  __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "%s", fmt)
...
int main (int argc, char** argv)
{
    ...
    MNLD_DATA_T *obj = &mnld_data;
    printf("MNLD started\n");
    MND_MSG("MNLD started\n");
    if ((argv[2]==NULL || !strncmp(argv[2], "meta", 4)) || (argv[2]==NULL || !strncmp(argv[2], "factory", 4)) || (argv[0]==NULL || !strncmp(argv[0], "test", 4))){
	printf("Meta or factory or adb shell mode\n");
		...
		printf("Meta or factory mode exit\n");
                exit_meta_factory = 0;
                exit(1);
            }
        ...
        if(agps2mnld_fd == -1){
        	printf("agps2mnld create socket fail: %d (%s)\n", errno, strerror(errno));
        }else{
		...
		if(mtklogger2mnld_fd == -1){
        	printf("mtklogger2mnld_fd create socket fail: %d (%s)\n", errno, strerror(errno));
        }else{       	
			printf("mtklogger2mnld_fd created success\n");
        ...
        while(1) {
          ...
          for (;;) {
              ...
              if (err == ERR_REMOTE_HANGUP) {
                  printf("remote hangup (cleanup?), wait for new connection\n");
                  break;
              } else if (errno == EINTR) {
                  continue;
              } else if (err) {
                  printf("process data error: %d (%s)\n", errno, strerror(errno));
              }
          }
